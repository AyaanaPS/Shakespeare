...
HMM = pickle.load(open('HMM_2_4_20.pkl','rb'))
word_map2 = pickle.load(open('word_map2.pkl','rb'))
tag_map = pickle.load(open('tag_map.pkl','rb'))
word_type = pickle.load(open('word_type.pkl','rb'))
...
def convertWordID(sequence):
	new_sequence = [sequence[0]]
	for i in range(1, len(sequence)):
		tag = tag_map[sequence[i]]
		wordOptions = word_type[tag]
...
for nRhymes in range(7):
	while True:
		nSyl1, nSyl2 = 0,0
		rhymeline1, rhymeline2 = HMM.generate_rhymingLines(M)
		rhymeline1 = convertWordID(rhymeline1)
		rhymeline2 = convertWordID(rhymeline2)
		...
		if (nSyl1 == 10) and (nSyl2 == 10):
			...
			text1 += ' ' + word_map2[rhymeline1[0]]
			text2 += ' ' + word_map2[rhymeline2[0]]
			break
