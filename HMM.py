...
class HiddenMarkovModel:
    ...
    def generate_emission(self, M):
        ...
        stateprobs1 = self.O[:,rhyme_ind1]
        stateprobs2 = self.O[:,rhyme_ind2]
        ## just observation probability to start, no transition probability
        state1 = np.random.choice( self.L, p = stateprobs1 / np.sum(stateprobs1) )
        state2 = np.random.choice( self.L, p = stateprobs2 / np.sum(stateprobs2) )
        states1 += [ state1 ]
        states2 += [ state2 ]
        
        ## indices for next word/state
        ii,jj = np.meshgrid( range(self.L), range(self.D) )
        nSamples = self.D*self.L
        ii = ii.reshape( nSamples )
        jj = jj.reshape( nSamples )
        
        for i in range(1,M):

            ## total probabilities for next word
            probs1 = np.zeros( self.O.shape )                
            probs2 = np.zeros( self.O.shape )                

            ## transition probabilities for next word
            trans_probs1 = self.A[ states1[i-1] ]
            trans_probs2 = self.A[ states2[i-1] ]

            ## include observed and transitional probabilities
            for j in range( self.D ):
                probs1[:,j] = self.O[:,j] * trans_probs1
                probs2[:,j] = self.O[:,j] * trans_probs2
                
            probs_1 = probs1.reshape( nSamples ) / np.sum(probs1)
            probs_2 = probs2.reshape( nSamples ) / np.sum(probs2)

            sample_ind1 = np.random.choice( nSamples, p = probs_1 )
            sample_ind2 = np.random.choice( nSamples, p = probs_2 )

            seq1 = jj[sample_ind1]
            seq2 = jj[sample_ind2]

            state1 = ii[sample_ind1]
            state2 = ii[sample_ind2]
